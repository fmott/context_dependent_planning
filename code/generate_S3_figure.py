...
filename = glob.glob('../data/model/model_comparison_alternative_HM_PM_and_complex_heuristics.csv') 
looic_df = pd.read_csv(filename[0],index_col = 0)
# Drop models with convergence issues. Note: these model would not change the results.
looic_df = looic_df.drop(labels=['HM OCR+S','AVGRR+S'], axis='index')
...
ax.set_xlabel('LOOIC')
ax.set_xlim(3200,6000) 

timestr = time.strftime("%Y%m%d%H%M%S")
# fig.savefig('S3_fig_' + timestr + '.tif', dpi=300, bbox_inches='tight', transparent=False)